...
public class HugeFungusFeature extends Feature<HugeFungusConfiguration> {
   ...
   private static boolean isReplaceable(WorldGenLevel pLevel, BlockPos pPos, HugeFungusConfiguration pConfig, boolean p_285162_) {
      if (pLevel.isStateAtPosition(pPos, BlockBehaviour.BlockStateBase::canBeReplaced)) {
         return true;
      } else {
         return p_285162_ ? pConfig.replaceableBlocks.test(pLevel, pPos) : false;
      }
   }
   ...
   private void placeHat(WorldGenLevel pLevel, RandomSource pRandom, HugeFungusConfiguration pConfig, BlockPos pPos, int p_285156_, boolean p_285265_) {
      ...
      int i = Math.min(pRandom.nextInt(1 + p_285156_ / 3) + 5, p_285156_);
      int j = p_285156_ - i;

      for(int k = j; k <= p_285156_; ++k) {
         int l = k < p_285156_ - pRandom.nextInt(3) ? 2 : 1;
         if (i > 8 && k < j + 4) {
            l = 3;
         }
