...
public class BeeStingerLayer<T extends LivingEntity, M extends PlayerModel<T>> extends StuckInBodyLayer<T, M> {
   ...
   private static void vertex(VertexConsumer pConsumer, Matrix4f p_254513_, Matrix3f p_254052_, float pX, int pY, float pU, float pV, int pLightmapUV) {
      pConsumer.vertex(p_254513_, pX, (float)pY, 0.0F).color(255, 255, 255, 255).uv(pU, pV).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pLightmapUV).normal(p_254052_, 0.0F, 1.0F, 0.0F).endVertex();
   }
}