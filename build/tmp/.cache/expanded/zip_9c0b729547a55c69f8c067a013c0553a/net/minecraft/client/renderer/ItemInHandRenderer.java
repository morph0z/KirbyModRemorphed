...
public class ItemInHandRenderer {
   ...
   private void applyEatTransform(PoseStack pMatrixStack, float pPartialTicks, HumanoidArm pHand, ItemStack pStack) {
      ...
      if (f1 < 0.8F) {
         ...
         pMatrixStack.translate(0.0F, f2, 0.0F);
      }

      float f3 = 1.0F - (float)Math.pow((double)f1, 27.0D);
      int i = pHand == HumanoidArm.RIGHT ? 1 : -1;
      pMatrixStack.translate(f3 * 0.6F * (float)i, f3 * -0.5F, f3 * 0.0F);
      pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)i * f3 * 90.0F));
      pMatrixStack.mulPose(Axis.XP.rotationDegrees(f3 * 10.0F));
      pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)i * f3 * 30.0F));
   }
