...
public class ChunkHeightAndBiomeFix extends DataFix {
   ...
   private Dynamic<?> predictChunkStatusBeforeSurface(Dynamic<?> pDynamic, Set<String> p_184905_) {
   ...
   private static Dynamic<?>[] getBiomeContainers(Dynamic<?> pDynamic, boolean p_184908_, int p_184909_, MutableBoolean p_184910_) {
   ...
   private static Dynamic<?> updateChunkTag(Dynamic<?> pChunkTag, boolean p_184913_, boolean p_184914_, boolean p_184915_, Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> p_184916_) {
   ...
   private static Dynamic<?> makePalettedContainer(Dynamic<?> pDynamic) {
      return pDynamic.createMap(ImmutableMap.of(pDynamic.createString("palette"), pDynamic));
   }

   private static Dynamic<?> makePalettedContainer(Dynamic<?> pDynamic, Dynamic<?> p_184893_) {
      return pDynamic.createMap(ImmutableMap.of(pDynamic.createString("palette"), pDynamic, pDynamic.createString("data"), p_184893_));
   }

   private static Dynamic<?> makeOptimizedPalettedContainer(Dynamic<?> pDynamic, Dynamic<?> p_184960_) {
      List<Dynamic<?>> list = pDynamic.asStream().collect(Collectors.toCollection(ArrayList::new));
      if (list.size() == 1) {
         return makePalettedContainer(pDynamic);
      } else {
         pDynamic = padPaletteEntries(pDynamic, p_184960_, list);
         return makePalettedContainer(pDynamic, p_184960_);
      }
   }
