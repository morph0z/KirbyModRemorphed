...
public abstract class BiomeSource implements BiomeResolver {
   ...
   @Nullable
   public Pair<BlockPos, Holder<Biome>> findClosestBiome3d(BlockPos pPos, int p_220579_, int p_220580_, int p_220581_, Predicate<Holder<Biome>> pBiomePredicate, Climate.Sampler pSampler, LevelReader pLevel) {
      ...
      if (set.isEmpty()) {
         ...
         int[] aint = Mth.outFromOrigin(pPos.getY(), pLevel.getMinBuildHeight() + 1, pLevel.getMaxBuildHeight(), p_220581_).toArray();
         ...
         for(BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.spiralAround(BlockPos.ZERO, i, Direction.EAST, Direction.SOUTH)) {
            int j = pPos.getX() + blockpos$mutableblockpos.getX() * p_220580_;
            int k = pPos.getZ() + blockpos$mutableblockpos.getZ() * p_220580_;
