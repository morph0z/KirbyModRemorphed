...
public class OverreachingTickFix extends DataFix {
   ...
   private static Dynamic<?> extractOverreachingTicks(Dynamic<?> pTag, int p_207664_, int p_207665_, Optional<? extends Dynamic<?>> p_207666_, String p_207667_) {
      if (p_207666_.isPresent()) {
         ...
         if (!list.isEmpty()) {
            pTag = pTag.set("UpgradeData", pTag.get("UpgradeData").orElseEmptyMap().set(p_207667_, pTag.createList(list.stream())));
         }
      }
